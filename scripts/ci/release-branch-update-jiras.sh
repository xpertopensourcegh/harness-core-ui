...
echo $VERSION
KEYS=`git log --pretty=oneline --abbrev-commit | awk "1;/Branching to release\//{exit}" | grep -o -iE '(BT|CCE|CCM|CDC|CDNG|CDP|CE|CI|COMP|CV|CVNG|DEL|DOC|DX|ER|FFM|OPS|PIE|PL|SEC|SWAT|GTM|ONP)-[0-9]+' | sort | uniq`
echo --- List of JIRA ---
echo $KEYS
echo --- End
