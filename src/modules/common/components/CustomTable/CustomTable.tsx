...
function GlobalFilter({ globalFilter, setGlobalFilter }: any) {
  ...
  return (
    <TextInput
      ...
      rightElement={isOpen && ((<Icon name="cross" margin="none" onClick={() => setIsOpen(false)} />) as any)}
    />
  )
}
...
const Table = ({ columns, data, openFilterPanel }: any): JSX.Element => {
  ...
  return (
    <section className={css.tableWrapper}>
      <Layout.Horizontal flex={true} spacing="xlarge" padding="medium" style={{ justifyContent: 'flex-end' }}>
                              ...
                              <div style={{ display: 'grid', alignItems: 'center', gridGap: '5px' }}>
                                <div>
                                  ...
                                  {cell.row.original?.tags?.split(',').length > 0 && (
                                    ...
                                    >
                                      <span>
                                        <Icon name="main-tags" size={16} style={{ marginLeft: 5, paddingBottom: 3 }} />{' '}
