...
import * as yamlService from '@wings-software/language-server/dist/languageservice/yamlLanguageService'
...
export class YAMLWorker {
  ...
  public format(uri: string, range: ls.Range, options: yamlService.CustomFormatterOptions): Thenable<ls.TextEdit[]> {
    const document = this._getTextDocument(uri)
