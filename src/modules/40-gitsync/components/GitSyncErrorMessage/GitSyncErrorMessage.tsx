...
const CountComponent: React.FC<{ count?: number }> = ({ count }) => {
  return (
    <Layout.Horizontal margin={{ right: 'medium' }}>
      ...
      <Text font={{ weight: 'bold' }} color={Color.RED_600}>
        {count}
      </Text>
    </Layout.Horizontal>
  )
...
const IconValue: React.FC<{ icon: IconName; value?: string; background?: Color; color?: Color }> = ({
  icon,
  value,
  background,
  color
  ...
  return (
    ...
    >
      ...
      <Text font={{ weight: 'semi-bold', size: 'small' }} color={color || Color.GREY_900}>
        {value}
      </Text>
    </Layout.Horizontal>
  )
...
const TimestampComponent: React.FC<{ timestamp?: number }> = ({ timestamp }) => {
  return (
    <Layout.Horizontal className={styles.timestamp} padding={{ left: 'medium' }}>
...
export const GitSyncErrorMessage: React.FC<GitSyncErrorMessageProps> = props => {
  ...
  return (
    <Card className={styles.gitSyncErrorMessage}>
      <Layout.Vertical>
        <Layout.Horizontal className={styles.section} flex={{ justifyContent: 'space-between' }}>
          <Text font={{ weight: 'bold' }} color={Color.GREY_900}>
            {title}
          </Text>
          <Layout.Horizontal flex={{ alignItems: 'center' }}>
            {renderComponentConditionally(<CountComponent count={count} />, !!count)}
            {renderComponentConditionally(<IconValue icon={'repository'} value={repo} />, !!repo)}
            {renderComponentConditionally(<IconValue icon={'git-new-branch'} value={branch} />, !!branch)}
            {renderComponentConditionally(
              <IconValue icon={'git-commit'} value={commitId} background={Color.PRIMARY_1} color={Color.PRIMARY_7} />,
              !!commitId
            )}
