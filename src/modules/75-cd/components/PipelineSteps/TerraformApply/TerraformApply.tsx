...
export interface TerraformApplyVariableProps {
  initialValues: TerraformData
...
function TerraformApplyWidget(
  props: TerraformApplyProps,
    ...
    <>
      <Formik<TerraformData>
        onSubmit={(values: TerraformData) => {
          if (values?.spec?.configuration?.type === ConfigurationTypes.Inline) {
...
const TerraformApplyInputStep: React.FC<TerraformApplyProps> = ({ inputSetData, readonly }) => {
  const { getString } = useStrings()
  return (
    <>
...
const TerraformApplyVariableStep: React.FC<TerraformApplyVariableProps> = ({
  variablesData,
  metadataMap,
  initialValues
}) => {
...
const TerraformApplyWidgetWithRef = React.forwardRef(TerraformApplyWidget)
...
export class TerraformApply extends PipelineStep<TerraformData> {
  ...
  renderStep(props: StepProps<TerraformData, unknown>): JSX.Element {
    ...
    if (stepViewType === StepViewType.InputSet || stepViewType === StepViewType.DeploymentForm) {
      ...
      return (
        <TerraformApplyVariableStep
          {...(customStepProps as TerraformApplyVariableProps)}
          initialValues={initialValues}
          onUpdate={onUpdate}
        />
      )
