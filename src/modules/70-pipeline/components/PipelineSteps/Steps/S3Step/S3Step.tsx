...
import css from './S3Step.module.scss'
...
export interface S3StepWidgetProps {
  initialValues: any //S3StepData
  onUpdate?: (data: any) => void //S3StepData
  stepViewType?: StepViewType
}

const S3StepWidget: React.FC<S3StepWidgetProps> = ({ initialValues, onUpdate }): JSX.Element => {
  const {
    state: { pipelineView },
    updatePipelineView
        ...
        {({ values: formValues, setFieldValue, handleSubmit }) => (
          <FormikForm>
            ...
            <div className={css.fieldsSection}>
              ...
              <Text margin={{ bottom: 'xsmall' }}>{getString('pipelineSteps.regionLabel')}</Text>
              <div className={cx(css.fieldsGroup, css.withoutSpacing, css.bottomSpacing)}>
                <FormInput.MultiTextInput name="spec.region" label="" style={{ flexGrow: 1 }} />
                {getMultiTypeFromValue(formValues.spec.region) === MultiTypeInputType.RUNTIME && (
                  <ConfigureOptions
                    value={formValues.spec.region as string}
                    type={
                      <Layout.Horizontal spacing="medium" style={{ alignItems: 'center' }}>
                        <Text>{getString('pipelineSteps.regionLabel')}</Text>
                      </Layout.Horizontal>
                    }
                    ...
                    showDefaultField={false}
                    showAdvanced={true}
                    onChange={value => setFieldValue('spec.region', value)}
                  />
                )}
              </div>
              <Text margin={{ bottom: 'xsmall' }}>{getString('pipelineSteps.pullLabel')}</Text>
              <div className={cx(css.fieldsGroup, css.withoutSpacing, css.bottomSpacing)}>
...
export class S3Step extends PipelineStep<any /*S3StepData*/> {
  renderStep(
    initialValues: any, //S3StepData
    onUpdate?: (data: any) => void, //S3StepData
    stepViewType?: StepViewType
  ...
  protected defaultValues: any /*S3StepData*/ = {
    identifier: '',
    spec: {}
  }
}
