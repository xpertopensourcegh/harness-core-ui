...
describe('<PipelineDeploymentListHeader /> test', () => {
  ...
  test('test deployment list tabs by click on those tabs', async () => {
    ...
    expect(getByTestId(renderedComponent, 'location')).toMatchInlineSnapshot(`
      ...
      >
        /account/testAcc/cd/orgs/testOrg/projects/test/pipelines?status=Aborted
      </div>
    `)
    ...
    expect(getByTestId(renderedComponent, 'location')).toMatchInlineSnapshot(`
      <div
        data-testid="location"
      >
        /account/testAcc/cd/orgs/testOrg/projects/test/pipelines?status=Aborted&pipelineIdentifier=pipeline1
      </div>
    `)
  })
})
